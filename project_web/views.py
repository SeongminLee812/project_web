...
def search(request):
    ...
    if len(search_word_split) > 1:
        search_result_kmooc = MoocAllList.objects.filter(Q(name__icontains=search_word_split[0])|Q(name__icontains=search_word_split[1])).order_by('-enrollment_start')
    ...
    # 각 데이터 베이스마다 이름을 나타내는 컬럼명이 다르므로, 각각 따로 검색 기능 구현함
        ...
        search_result_kmooc = MoocAllList.objects.filter(name__icontains=search_word).order_by('-enrollment_start')
